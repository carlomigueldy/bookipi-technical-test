...
#
# infra/api.Dockerfile — @flash/api (NestJS on Fastify)
...
# ---- build: full source, build @flash/api and its workspace dependencies ----
FROM deps AS build
COPY . .
RUN pnpm --filter @flash/api... build
...
RUN pnpm --filter=@flash/api --prod deploy --legacy /prod/api
